...
calcBtn.addEventListener("click", function () {
  const selectedElementSymbol = inputdrop1.value;
  const selectedElement = elements.find(
    (el) => el.value === selectedElementSymbol
  );
  const atomicNumber = selectedElement.atomicNumber;
  const atomicMass = selectedElement.atomicMass;
  const configuration = getElectronConfiguration(atomicNumber);
  const valenceElectrons = getValenceElectrons(configuration);

  result.innerHTML = `
      <p>Atomic number: ${atomicNumber}</p>
      <p>Atomic mass: ${atomicMass}</p>
      <p>The electron configuration of element is:</p>
      <p>${configuration.replace(/\s/g, ", ")}</p>
      <p>The valence electrons are:</p>
      <p>${valenceElectrons.replace(/\s/g, ", ")}</p>
    `;
});
...
function getElectronConfiguration(atomicNumber) {
  ...
  for (let i = 0; i < orbitalOrder.length && atomicNumber > 0; i++) {
    const orbital = orbitalOrder[i];
    const orbitalType = orbital[1];
    const electronsInThisOrbital = Math.min(
      atomicNumber,
      maxElectronsInOrbital[orbitalType]
    );

    configuration += orbital + electronsInThisOrbital + " ";
    atomicNumber -= electronsInThisOrbital;
  }

  return configuration.trim();
}

function getValenceElectrons(configuration) {
  const shells = configuration.split(" ");
  const valenceShell = shells[shells.length - 1][0];
  const valenceElectrons = shells
    .filter((shell) => shell[0] === valenceShell)
    .join(" ");
